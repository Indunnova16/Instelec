...
class ExcelPresupuestoTests(TestCase):
    ...
    def test_hoja_resumen_trae_kpi_correctos(self):
        ...
        hoja = libro["Resumen"]
        valores = {
            fila[0].value: fila[1].value for fila in hoja.iter_rows(min_row=5) if fila[0].value
        }
...
class CsvPresupuestoTests(TestCase):
    ...
    def test_dato_legacy_sin_filas_detalle_no_produce_csv_vacio(self):
        """Hallazgo del validador-cierre (2026-09-23, regla R6): un
        presupuesto cargado por el importador CONTABLE legacy (SIN
        'filas_detalle' -- el caso REAL vigente en prod, proyecto Puerta de
        Oro) producía un CSV con SOLO encabezado (0 filas de datos), sin
        aviso al usuario. Fallback: una fila anual por rubro desde
        'rubro_rows' (mismos números que la hoja Excel 'Rubros')."""
        from django.http import HttpResponse

        proyecto_legacy = _crear_proyecto("Proyecto A10 CSV legacy")
        PresupuestoDetalladoConstruccion.objects.create(
            proyecto=proyecto_legacy,
            anio=2026,
            tipo=PresupuestoDetalladoConstruccion.Tipo.PLANEADO,
            datos={
                "finv2_bd": {
                    "total": -1000.0,
                    "rubros": {
                        "Personal": {"total": 7000.0, "meses": {"julio": 7000.0}, "cuentas": []},
                    },
                }
            },
        )
        response = HttpResponse(content_type="text/csv; charset=utf-8")
        escribir_csv_presupuesto(response, proyecto_legacy, 2026)
        contenido = response.content.decode("utf-8-sig")
        filas = list(csv.reader(io.StringIO(contenido)))
        datos_filas = [f for f in filas[1:] if f]
        self.assertEqual(len(datos_filas), 1)
        self.assertEqual(datos_filas[0][0], "SIN_HOMOLOGAR")
        self.assertEqual(datos_filas[0][1], "Personal")
        self.assertEqual(datos_filas[0][3], proyecto_legacy.nombre)
        self.assertEqual(float(datos_filas[0][5]), 7000.0)
...
class GateFormatoReporteIntegracionTests(TestCase):
    def setUp(self):
        ...
        self.urls = {
            "pdf": reverse(
                "construccion:fin_presupuesto_planeado_pdf",
                kwargs={"proyecto_id": self.proyecto.pk},
            ),
            "excel": reverse(
                "construccion:fin_presupuesto_planeado_excel",
                kwargs={"proyecto_id": self.proyecto.pk},
            ),
            "csv": reverse(
                "construccion:fin_presupuesto_planeado_csv",
                kwargs={"proyecto_id": self.proyecto.pk},
            ),
            "ppt": reverse(
                "construccion:fin_presupuesto_planeado_ppt",
                kwargs={"proyecto_id": self.proyecto.pk},
            ),
        }
    ...
    def test_content_types_correctos(self):
        ...
        self.assertIn(
            "application/pdf", client.get(self.urls["pdf"], {"anio": 2026})["Content-Type"]
        )
        self.assertIn(
            "spreadsheetml", client.get(self.urls["excel"], {"anio": 2026})["Content-Type"]
        )
        self.assertIn("csv", client.get(self.urls["csv"], {"anio": 2026})["Content-Type"])
        self.assertIn(
            "presentationml", client.get(self.urls["ppt"], {"anio": 2026})["Content-Type"]
        )
...
class BotonesReporteTemplateTests(TestCase):
    ...
    def setUp(self):
        ...
        _crear_presupuesto(self.proyecto)
        self.url = reverse(
            "construccion:fin_presupuesto_planeado", kwargs={"proyecto_id": self.proyecto.pk}
        )
