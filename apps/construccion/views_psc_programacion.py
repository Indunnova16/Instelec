...
from apps.construccion.services_psc_presupuesto import obtener_plan_presupuesto_para_real
...
class ProgramacionSemanalConstruccionForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if actual and not supervisor.queryset.filter(pk=actual.pk).exists():
            supervisor.queryset = (
                supervisor.queryset | Usuario.objects.filter(pk=actual.pk)
            ).order_by(
                "first_name",
                "last_name",
            )
...
class ProgramacionSemanalConstruccionCreateView(LoginRequiredMixin, RoleRequiredMixin, CreateView):
    model = ProgramacionSemanalConstruccion
...
class ProgramacionSemanalConstruccionUpdateView(LoginRequiredMixin, RoleRequiredMixin, UpdateView):
    model = ProgramacionSemanalConstruccion
    form_class = ProgramacionSemanalConstruccionForm
    template_name = "construccion/programacion_semanal/form.html"
    allowed_roles = PSC_ADMIN_ROLES
...
class ProgramacionSemanalConstruccionDetailView(LoginRequiredMixin, RoleRequiredMixin, DetailView):
    model = ProgramacionSemanalConstruccion
    ...
    def get_context_data(self, **kwargs):
        ...
        vehiculos_asignados = (
            ProgramacionSemanalConstruccionVehiculo.objects.filter(
                programacion=programacion,
            )
            .select_related("vehiculo", "conductor")
            .order_by("vehiculo__placa")
        )
        conductores_disponibles = personal_asignado.filter(
            personal__activo=True,
            personal__area="CONSTRUCCION",
        ).select_related("personal__rol_cuadrilla")
        plan_presupuesto = obtener_plan_presupuesto_para_real(programacion.pk)
        context.update(
            {
                # B6 — contrato de contexto para los partials de asignación.
                "personal_asignado": personal_asignado,
                # B1/B2 (#225 Sprint B) — composición presupuestada + estado congelado/editable.
                "plan_presupuesto": plan_presupuesto,
                "presupuesto_congelado": plan_presupuesto.congelado_en is not None,
