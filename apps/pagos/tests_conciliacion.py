...
completado). Port del canonico pagos-template (PR #13). Cubre: detección
por prefijo, exclusión de pagos ya registrados, exclusión de otras apps
del merchant compartido, y el endpoint cron de solo-detección.
"""
...
@override_settings(WOMPI_REFERENCE_PREFIX='INSTELEC')
class DetectarFaltantesTest(TestCase):
    def setUp(self):
        self.plan = PlanServicio.objects.create(nombre='Plan', precio=Decimal('75000'))
        self.suscripcion = Suscripcion.objects.create(plan=self.plan)

    @patch('apps.pagos.conciliacion.wompi.listar_transacciones')
    def test_detecta_aprobada_sin_registro_local(self, mock_list):
        mock_list.return_value = [_tx('tx-perdida', 'INSTELEC-1-1-202608-1M')]
    ...
    def test_ignora_pago_ya_registrado(self, mock_list):
        ...
        mock_list.return_value = [_tx('tx-registrada', 'INSTELEC-1-1-202608-1M')]
    ...
    def test_ignora_otras_apps_del_merchant_compartido(self, mock_list):
        ...
        # prefijo propio (INSTELEC-) cuentan.
    ...
    def test_ignora_no_aprobadas(self, mock_list):
        mock_list.return_value = [
            _tx('tx-declinada', 'INSTELEC-1-1-202608-1M', status='DECLINED'),
            _tx('tx-pendiente', 'INSTELEC-1-1-202608-1M', status='PENDING'),
        ]
...
class CronConciliarWompiTest(TestCase):
    ...
    @patch('apps.pagos.conciliacion.wompi.listar_transacciones', side_effect=Exception('API caida'))
