...
(prefijo WOMPI_REFERENCE_PREFIX, INSTELEC) y no tienen Pago local — el
escenario que dejó un pago real de Obraje cobrado-pero-invisible en
jul-2026 (webhook roto + redirect no completado).
...
from apps.pagos import alegra
from apps.pagos.conciliacion import detectar_faltantes
from apps.pagos.models import Pago, Suscripcion, calcular_n_meses
from apps.pagos.views import _avanzar_fecha_proximo_pago

# Mismo mapa del webhook local (WompiWebhookView) — VOIDED es ANULADO aca,
# no RECHAZADO como en el template canonico.
ESTADO_MAP = {
    'APPROVED': 'APROBADO',
    'DECLINED': 'RECHAZADO',
    'ERROR': 'ERROR',
...
class Command(BaseCommand):
    ...
    def _registrar(self, tx):
        """Misma lógica idempotente del webhook local para un APPROVED recuperado.

        Replica la secuencia post-APPROVED de WompiWebhookView: get_or_create
        por wompi_transaction_id (con n_meses via calcular_n_meses, issue #199),
        guard ya_estaba_aprobado (issue #195), activar suscripcion + avanzar
        fecha_proximo_pago, y factura Alegra si no existe.
        """
        tx_id = tx.get('id')
        ...
        if not pago.alegra_invoice_id:
            try:
                ...
                factura = None
                self.stderr.write(f'  Error generando factura Alegra: {e}')
            if factura:
                self.stdout.write(self.style.SUCCESS(
